# 02 - PublicEndpointCanaryFailed

Tao alarm `budget-bot-hackathon-PublicEndpointCanaryFailed`.
...
aws cloudwatch put-metric-alarm \
  --region "$AWS_REGION" \
  --alarm-name "${ALARM_PREFIX}-PublicEndpointCanaryFailed" \
  --alarm-description "Public endpoint canary failed" \
  --namespace CloudWatchSynthetics \
  --metric-name Failed \
  --dimensions Name=CanaryName,Value="$CANARY_NAME" \
  --statistic Sum \
...
aws cloudwatch describe-alarms \
  --region "$AWS_REGION" \
  --alarm-names "${ALARM_PREFIX}-PublicEndpointCanaryFailed" \
  --output table
```
...
aws cloudwatch list-tags-for-resource \
  --region "$AWS_REGION" \
  --resource-arn "arn:aws:cloudwatch:${AWS_REGION}:${AWS_ACCOUNT_ID}:alarm:${ALARM_PREFIX}-PublicEndpointCanaryFailed" \
  --output table
```
